...
def resheto_Atkin():
    ...
    for i in range(1,sqr_lim+1):
        x=i*i #Инициализация числа х
        for j in range(1,sqr_lim+1):
            y=j*j #Инициализация числа у
            n=4*x+y
    ...
    for i in range(10**6,limit+1):#Присваивание списку элементы простых чисел 
        if prime_boolean[i]:
...
class Notepad(QWidget):
    ...
    def shifr_text(self):
            for i in range(len(self.text_t_s)):
                ...
                self.new_shifred+=str(chisl_form)
                self.new_shifred+=' '
    ...
    def deshifr_text(self):
        ...
        for i in range(len(self.text_t_s)):
            if self.text_t_s[i]!=' ':
                de_number+=self.text_t_s[i]
            else:
                self.new_deshifred.append(chr(to_a_degree(int(de_number),d,self.p*self.q)))
                de_number=''
